...
8. **Static map preview (cemetery pages).** `src/components/CemeteryMap-Claude.astro` renders a small Maps Static API `<img>` (600x300, zoom 15, marker at the cemetery's real coordinates) on cemetery pages that have coordinates, next to the existing OSM/Google directions links. The URL is built as a plain string at build/render time from `import.meta.env.PUBLIC_GOOGLE_MAPS_KEY`; the image itself is fetched live by each visitor's browser directly from Google, never fetched or cached by this build (Maps Static API ToS forbid baking the returned PNG into `dist/` for indefinite redistribution; a live per-request `<img>` is the compliant pattern). Renders nothing if the key is unset, same gating as `AffiliateNote-Claude.astro`. **Before production use, split into two separately-restricted Google Maps Platform API keys in Google Cloud Console** (project "kg-gsc-mcp"): one HTTP-referrer-restricted to `restingplaces.site` for this public embed, and a separate, non-referrer-restricted key for any future server-side Places API calls in `scripts/enrich-Claude.mjs` (build-time requests carry no browser Referer header, so a referrer-restricted key would fail there). Reusing one key for both breaks one of the two use cases; this is a Google Cloud Console action item for the site owner, not something code can fix. The key currently in local `.env` is unrestricted-by-referrer and provisional/test-only — same note is left inline in `.env` and in the component's header comment.
